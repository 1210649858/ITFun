<template>
    <div>
        ...
        <el-table :data="notice_nodes" style="width: 100% ;margin-top: 5px" @selection-change="check">
        ...
        <el-dialog title="编辑分类" :visible.sync="Edit">
            <el-form :model="notice_node" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                <el-form-item label="分类名" prop="name">
                    <el-input v-model="notice_node.name"></el-input>
                </el-form-item>

                <el-form-item label="排序" :label-width="formLabelWidth">
                    <el-input-number v-model="notice_node.sort" @change="handleChange" :min="1" :max="99"
                                     label="描述文字"></el-input-number>
                </el-form-item>
        ...
        methods: {
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post(`/admin/notice_nodes`, this.ruleForm)
                            .then((res) => {
                                this.Create = false;
                                this.$message({
                                    message: '(＾－＾)新增成功',
                                ...
                                this.init();
                                this.$router.push({name: 'notice_nodes'})
                            });
                    } else {
            ...
            handleEdit(index, row) {
                this.Edit = true;
                axios.get(`/admin/notice_nodes/${row.id}/edit`)
                    .then((res) => {
                        this.notice_node = res.data;
                    })
            },
            //编辑
            updateForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put(`/admin/notice_nodes/${this.notice_node.id}`, this.notice_node)
                            .then((res) => {
                                this.Edit = false;
                                this.$message({
                                    message: '(∩ _ ∩)编辑成功',
            ...
            handleDelete(index, row) {
                this.$confirm('亲 (●ﾟωﾟ●)确定要删除吗?', '提示', {
                    ...
                    axios.delete(`/admin/notice_nodes/${row.id}`)
                        .then((res) => {
                            this.$message({
                                type: 'success',
                                message: 'OK,删除成功!'
            ...
            destroyChecked(){
                if (this.Selection.length == 0) {
                    ...
                    this.$confirm('亲 (●ﾟωﾟ●)确定要删除吗?', '提示', {
                        ...
                        axios.post(`/admin/notice_nodes/destroy_checked`,{checked_id:checked_id})
                            .then(() => {
                                this.$message({
                                    type: 'success',
                                    message: 'OK,删除成功!'
            ...
            change(index, row) {
                axios.patch(`/admin/notice_nodes/change_sort`,row)
                    .then(() => {
                        this.$message({
                            type: 'success',
                            message: '排序成功!'
